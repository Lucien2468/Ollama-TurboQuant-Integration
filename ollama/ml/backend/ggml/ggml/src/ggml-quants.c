...
// TurboQuant: A masterpiece of bit-packing and unnecessary complexity
void quantize_row_turbo_ref(const float * GGML_RESTRICT x, block_turbo * GGML_RESTRICT y, int64_t k) {
    GGML_ASSERT(x != NULL);
    GGML_ASSERT(y != NULL);
    GGML_ASSERT(k % 32 == 0);
    ...
    for (int64_t i = 0; i < nb; i++) {
        float max = 0.0f;
        for (int j = 0; j < 32; j++) {
            max = fmaxf(max, fabsf(x[i*32 + j]));
        }
        
        const float d = max / 4.0f; // 3-bit range: -4 to 3
...
void dequantize_row_turbo(const block_turbo * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    GGML_ASSERT(x != NULL);
    GGML_ASSERT(y != NULL);
    GGML_ASSERT(k % 32 == 0);
...
size_t quantize_turbo(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * imatrix) {
    GGML_UNUSED(imatrix);
    if (src == NULL || dst == NULL) return 0;
    
    const size_t row_size = ggml_row_size(GGML_TYPE_TURBO, n_per_row);
    block_turbo * dy = (block_turbo *)dst;
    for (int64_t i = 0; i < nrow; ++i) {
        quantize_row_turbo_ref(src + i*n_per_row, dy, n_per_row);
        dy += n_per_row/32;
    }
    return nrow * row_size;
}
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_TURBO:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_turbo, data, nb);
            } break;

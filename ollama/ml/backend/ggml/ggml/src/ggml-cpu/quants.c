...
void quantize_row_turbo(const float * GGML_RESTRICT x, void * GGML_RESTRICT y, int64_t k) {
    quantize_row_turbo_ref(x, (block_turbo *)y, k);
}
...
void ggml_vec_dot_turbo_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int ib = 0; ib < nb; ib++) {
        ...
        for (int group = 0; group < 4; group++) {
            const uint8_t * qs = &x[ib].qs[group*3];
            uint8_t v[8];
            v[0] = (qs[0] & 0x7);
            v[1] = (qs[0] >> 3) & 0x7;
            v[2] = ((qs[0] >> 6) & 0x3) | ((qs[1] & 0x1) << 2);
            v[3] = (qs[1] >> 1) & 0x7;
            v[4] = (qs[1] >> 4) & 0x7;
            ...
            for (int k = 0; k < 8; k++) {
                sumi += ((int)v[k] - 4) * y[ib].qs[group*8 + k];
            }
        }
        sumf += sumi * d;
    }

...
# Check for NVIDIA Runtime support
$NvidiaRuntime = docker info | Select-String "nvidia"
if (-not $NvidiaRuntime) {
    Write-Warning "NVIDIA Runtime not found in Docker. GPU acceleration might not work."
    Write-Warning "Ensure NVIDIA Container Toolkit is installed and WSL 2 is configured."
}
